...
#include "KeeperPerceptRegistration.h"
#include "UKFPose2D.h"
#include "Tools/Debugging/DebugDrawings.h"


KeeperPerceptRegistration::KeeperPerceptRegistration(const CameraMatrix& cameraMatrix,
...
theCameraMatrix(cameraMatrix), theFieldDimensions(fieldDimensions),
...
theFieldLines(fieldLines), theMidCorner(midCorner), theMotionInfo(motionInfo), theOuterCorner(outerCorner),
...
lineAssociationCorridor(lineAssociationCorridor),
...
void KeeperPerceptRegistration::update(const Pose2f& theRobotPose, RegisteredPercepts& registeredPercepts,
...
int KeeperPerceptRegistration::registerPoses(std::vector<RegisteredPose>& poses)
{
...
int KeeperPerceptRegistration::registerPose(std::vector<RegisteredPose>& poses, const FieldFeature& feature)
{
	if (feature.isValid)
	{
...
bool KeeperPerceptRegistration::pickPoseFromFieldFeature(const Pose2f& robotPose, const FieldFeature& fieldFeature, Pose2f& pickedPose) const
{
...
int KeeperPerceptRegistration::registerLandmarks(std::vector<RegisteredLandmark>& landmarks)
{
...
int KeeperPerceptRegistration::registerLines(std::vector<RegisteredLine>& lines)
{
...
bool KeeperPerceptRegistration::getAssociatedIntersection(const FieldLineIntersections::Intersection& intersection, Vector2f& associatedIntersection) const
{
...
bool KeeperPerceptRegistration::getAssociatedPenaltyMark(const Vector2f& penaltyMarkPercept, Vector2f& associatedPenaltyMark) const
{
...
bool KeeperPerceptRegistration::getAssociatedUnknownGoalPost(const Vector2f& goalPercept, Vector2f& associatedPost) const
{
...
bool KeeperPerceptRegistration::getAssociatedKnownGoalPost(const Vector2f& goalPercept, bool isLeft, Vector2f& associatedPost) const
{
	const Vector2f postInWorld = robotPose * goalPercept;
	if (postInWorld.x() <= 0.f) // own half
	{
...
const KeeperPerceptRegistration::FieldLine* KeeperPerceptRegistration::getPointerToAssociatedLine(const Vector2f& start, const Vector2f& end) const
{
...
float KeeperPerceptRegistration::getSqrDistanceToLine(const Vector2f& base, const Vector2f& dir, float length, const Vector2f& point) const
{
...
bool KeeperPerceptRegistration::intersectLineWithLine(const Vector2f& lineBase1, const Vector2f& lineDir1,
...
float KeeperPerceptRegistration::getSqrDistanceToLine(const Vector2f& base, const Vector2f& dir, const Vector2f& point) const
{
	const float l = (point.x() - base.x()) * dir.x() + (point.y() - base.y()) * dir.y();
	return ((base + dir * l) - point).squaredNorm();
}

bool KeeperPerceptRegistration::goalPostIsValid(const Vector2f& observedPosition, const Vector2f& modelPosition,
...
void KeeperPerceptRegistration::draw()
{
