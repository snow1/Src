...
thread_local TeammateDataProvider* TeammateDataProvider::theInstance = nullptr;
...
void TeammateDataProvider::setCurrentTeammate(int robotNumber)
{
  ...
  if((robotNumber == theRobotInfo.number) || (robotNumber < Settings::lowestValidPlayerNumber) ||
     (robotNumber > Settings::highestValidPlayerNumber))
  {
    currentTeammate = 0;
    return;
  }
...
bool TeammateDataProvider::handleMessage(InMessage& message)
{
  switch(message.getMessageID())
  {
	...
	case idnum:
		int robotNumbers;
		message.bin >> robotNumbers;
		//OUTPUT(idText, text, "key  " << robotNumbers);
		if (currentTeammate)
			currentTeammate->rolenumber = robotNumbers;
	  return true;
    ...
    case idDropInPlayer:
      if(currentTeammate)
      {
        ...
        currentTeammate->timeWhenLastPacketReceived = lastReceivedTimestamp = Time::getCurrentSystemTime();
        bool fallen;
        message.bin >> fallen;
    ...
    case idObstacleModelCompressed:
      if(currentTeammate)
      {
        ...
        for(auto& obstacle : currentTeammate->obstacleModel.obstacles)
        {
          REMOTE_TO_LOCAL_TIME(obstacle.lastSeen);
        }
      }
      return true;
    case idRobotPose:
	...
	/*case idRole:
	  if (currentTeammate)
		message.bin >> currentTeammate->role;
	  return true;*/
    case idTeammateFieldCoverage:
      if(currentTeammate)
      {
        UNPACK(FieldCoverageLine, fieldCoverageLine);
...
void TeammateDataProvider::update(UnfilteredTeammateData& unfilteredTeammateData)
{
  updateData(unfilteredTeammateData);
}

void TeammateDataProvider::updateData(TeammateData& teammateData)
{
  // Initialize:
  ...
  {
    if((!Global::getSettings().isDropInGame && teammate.isPenalized) || theOwnTeamInfo.players[teammate.number - 1].penalty != PENALTY_NONE)
