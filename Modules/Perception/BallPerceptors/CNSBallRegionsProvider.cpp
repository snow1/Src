...
void CNSBallRegionsProvider::update(BallRegions& ballRegions)
{
  ...
  DECLARE_DEBUG_DRAWING("module:CNSImageProvider:kuang", "drawingOnImage");
  ...
  for(const Vector2i& spot : spots)
  {
    ...
    CIRCLE("module:CNSBallRegionsProvider:expectedRadius", spot.x(), spot.y(), expectedRadius, 1, Drawings::solidPen, ColorRGBA::green, Drawings::noBrush, ColorRGBA::green);
    ...
    if(!predicted)
    {
      ...
      const Rangei xRange(upperLeft.x() / blockSizeX, lowerRight.x() / blockSizeX + 1);
      const Rangei yRange(upperLeft.y() / blockSizeY, lowerRight.y() / blockSizeY + 1);
	  LINE("module:CNSImageProvider:kuang", spot.x() - radius, spot.y() - radius, spot.x() - radius+40, spot.y() - radius,
