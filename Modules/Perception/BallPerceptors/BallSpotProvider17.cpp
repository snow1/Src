...
void BallSpotProvider17::update(BallSpots& ballSpots)
{
  ...
  DECLARE_DEBUG_DRAWING("module:BallSpotProvider17:currentLengthNeeded", "drawingOnImage");
        ...
        if(currentLengthNeeded != 0)
        {
          ...
          if(lowestYOfCurrendArea - region.range.lower > currentLengthNeeded)
            ballSpots.ballSpots.emplace_back(circle.center.cast<int>());
			  ...
			  if(currentLengthNeeded != 0 && lowestYOfCurrendArea == theColorScanlineRegionsVerticalClipped.scanlines[scanLineIndex].regions.front().range.lower &&
...
float BallSpotProvider17::getNeededLengthFor(const int x, const int y, Geometry::Circle& circle) const
{
  ...
  { 
	  LINE("module:BallSpotProvider17:currentLengthNeeded", x, y, x+ circle.radius + (startPoint - circle.center).norm() * ballSpotDistUsage, y-(circle.radius + (startPoint - circle.center).norm() * ballSpotDistUsage),
